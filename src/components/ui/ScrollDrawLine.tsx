...
/**
 * ScrollDrawLine — organic SVG path that draws itself as you scroll past it.
 * The stroke-dashoffset is driven directly by scroll position.
 */
...
export default function ScrollDrawLine({
  ...
  // Measure length after paint (rAF ensures SVG is rendered)
  useEffect(() => {
    const measure = () => {
      ...
      if (len === 0) {
        // retry next frame if not painted yet
      ...
      // Run scroll handler now so it's correct on first paint
      update()
    }
    requestAnimationFrame(measure)
  ...
  const update = () => {
    ...
    // Line starts drawing when its top enters the screen bottom,
    // finishes when its top reaches 25% from the top.
